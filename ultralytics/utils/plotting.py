...
def crop_and_encode(xyxy, im, gain=1.02, pad=10, square=False, BGR=False):
    """Crop an image according to the given bounding box and encode it as a base64 string.

    Args:
        xyxy (torch.Tensor or list): Bounding box coordinates in [x1, y1, x2, y2] format.
        im (numpy.ndarray): The input image.
        gain (float, optional): Factor to increase box size. Defaults to 1.02.
        pad (int, optional): Padding to add around the bounding box. Defaults to 10.
        square (bool, optional): If True, make the bounding box square. Defaults to False.
        BGR (bool, optional): Whether to use BGR or RGB. Defaults to False (RGB).

    Returns:
        str: Base64 encoded image.
    """
    # If bounding box is a list, convert it to tensor
    ...
    xyxy = b.long()
    h, w = im.shape[:2]
    xyxy[:, 0] = max(0, xyxy[:, 0].item())
    xyxy[:, 1] = max(0, xyxy[:, 1].item())
    xyxy[:, 2] = min(w, xyxy[:, 2].item())
    xyxy[:, 3] = min(h, xyxy[:, 3].item())

    # Crop the image using the bounding box
    crop = im[xyxy[0, 1] : xyxy[0, 3], xyxy[0, 0] : xyxy[0, 2], :: (1 if BGR else -1)]  # Adjust for BGR or RGB

    # Convert the cropped image (numpy array) to a PIL Image for easier manipulation
