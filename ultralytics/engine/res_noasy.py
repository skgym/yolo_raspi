...
from ultralytics.utils.plotting import Annotator, colors, crop_and_encode
...
class Results(SimpleClass):
    ...
    def save_crop(self, save_dir, file_name=Path("im.jpg"), frame_id=None):
        ...
        with open(coordinates_path, "w"):
            for d in self.boxes:
                ...
                f"{save_path.name}: {bbox[0].item()}, {bbox[1].item()}, {bbox[2].item()}, {bbox[3].item()}"

                filename = save_path.name
                cropped_image = crop_and_encode(
        ...
        for item in send_llava:
            self.process_image(self.url, item["image"], item["filename"])

        # asyncio.run(self.test_concurrent(send_llava))
        # for item in send_llava:
        #     asyncio.create_task(self.add_to_queue(item))
    ...
    #                 resp.raise_for_status()  # Ensure successful request
    ...
    def process_image(self, url, image, image_name):
        # prompt = "<image>\nPlease generate caption towards this image."
        prompt = '<image>\nPlease generate clear descriptions of the objects in the input image in this format: "A white SUV. A woman wearing a red top and blue pants. A man in a red shirt riding a black motorcycle."'
        ...
        response = self.send_request(  # await
            url + "/generate",
            {
        ...
        print(f"Filename: {image_name} - Caption: {response['text']}")
        return image_name, response["text"]

    async def test_concurrent(self, send_llava):

        tasks = [self.process_image(self.url, item["image"], item["filename"]) for item in send_llava]
